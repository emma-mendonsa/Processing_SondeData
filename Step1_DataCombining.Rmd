...
SiteData$`Time (HH:mm:ss)` = hms::as_hms(SiteData$`Time (HH:mm:ss)`) #Update Time column format
#SiteData$`Site Name`<- siteName  #If more than one, review and adjust in excels. Possibly run this line

#Add DateTime column
SiteData <- SiteData %>%
                      mutate(DateTime = as.POSIXct(paste(`Date (MM/DD/YYYY)`, `Time (HH:mm:ss)`), 
                                                   format = "%Y-%m-%d %H:%M:%S"))
...
SiteData <- SiteData %>% 
  select(`Date (MM/DD/YYYY)`,`Time (HH:mm:ss)`,DateTime, `Site Name`,
         `Depth m`,
         `Temp °C`,`SpCond µS/cm`,
         `ODO % sat`,`ODO mg/L`,
         `Turbidity FNU`,
...
write_rds(SiteData, file = paste("siteData/",siteName,"_combo_raw.rds",sep=""))
